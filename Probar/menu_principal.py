...
from layouts import layout_generar_collage as layout
...
def window_menu (perfil):
    ...
    """
        if event == 'editar_perfil':
            ...
            window['editar_perfil'].update(image_data=abrir_fotos.abrir(os.path.join(ROOT_PATH,"fotos",perfil['Foto']),(200,200)),image_subsample=2)
        if event == 'generar_meme':
            seleccionar_template_meme.window_seleccionar_template(perfil["Alias"])
        if event == 'generar_collage':
            layout.layout_collage(perfil["Foto"],perfil["Alias"])
        if event == 'etiquetar':
