...
def etiquetar_imagenes(alias):
    ...
    while True:
        ...
        if event == "-CARPETA-":
            ...
            try:
                cargo_foto=True
                nombre_foto = os.path.relpath(os.path.join(values["-CARPETA-"], values["-ARCHIVOS-"][0]),'.') #traemos la foto seleccionada
                ...
                if buscar_foto(nombre_foto) != None:
                    ...
                    etiquetas=eval(datos_imagen[5])
                    if len(etiquetas) > 0:
                        window["-ETIQUETAS-"].update(" ".join(map(str, etiquetas)))          
                    else:
                        window["-ETIQUETAS-"].update(" ")             
                else:
        ...
        if event=='-ETIQUETAR-':
            if not cargo_foto:
                sg.popup('Cargue una foto primero')
            else:
