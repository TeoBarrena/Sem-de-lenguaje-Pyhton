...
def crear_csv_imagenes():
    ...
    if not (os.path.exists(os.path.join(constante.ROOT_PATH,"archivos",'imagenes.csv'))):
        with open(os.path.join(constante.ROOT_PATH,"archivos",'imagenes.csv'),'w', newline='') as archivo_csv:
            writer = csv.writer(archivo_csv)
...
def guardar_linea_csv(nombre_foto,descripcion,etiquetas,alias):
    try: 
        ...
        with open (os.path.join(constante.ROOT_PATH,"archivos",'imagenes.csv'),'a',newline='') as archivo:
